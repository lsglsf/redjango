...
import logging
from Cmdb.methods.common import FibonacciRpcClient,RabbitMQAPI
        ...
        if self.get_cookie('user') == None:
            ...
            if bridge:
                if client_data['fun']=='fun_file':
                    for i in client_data['t_host']:
                        try:
                            ...
                            try:
                                ...
                                self.waiters_d[self._id()] = rabbitmq_object
                                if rabbitmq_status:
                                #print client_data
                    ...
                    #self.waiters_d[self._id()] = rabbitmq_object
                    if rabbitmq_status:
                        test = FibonacciRpcClient(host=RABBITMQ['default']['HOST'], username=RABBITMQ['default']['USER'],
                                                  password=RABBITMQ['default']['PASSWORD'], httpwrite=self)
                ...
                if client_data['fun']=='fun_file':
                  ...
                  #  print client_data['s_host']
                   # print client_data['t_host']
                    #app_name = client_data['app']
                    app_id = client_data['id']
                    host_hostip = Asset.objects.get(ip=client_data['s_host'])
                    client_data['s_host'] = {}
